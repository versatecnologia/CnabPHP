...
class YamlLoad
{
            ...
            foreach ($fields as $current_name => $current_field) {
                ...
                if ($current_pos_start > $current_pos_end) {
                    throw new \DomainException(
                        "No campo $current_name a posição inicial ($current_pos_start) deve ser menor ou igual à posição final ($current_pos_end)"
                    );
                }
    ...
    public function loadFormat($cnab, $filename)
    {
        ...
        if ($banco != 104) {
            ...
            $filenameEspecifico = $this->formatPath . '/' . $cnab . '/' . $banco . '/sigcb/' . $filename . '.yml';
        }
